...
float channel1( in vec2 p){
	return worley(16.0 * p) * .25;
}

vec4 channel2( in vec2 p){
    return vec4(0.0);
}
vec4 channel2blur( in vec2 p){
	return vec4(0.0);
}
...
float mapShell( in vec3 p, out vec4 matInfo ) {
    ...
    float di = channel2( vec2(t+r,0.5) ).x;
    d += 0.002*di;
...
vec3 shadeOpaque( in vec3 ro, in vec3 rd, in float t, in float m, in vec4 matInfo ){
    ...
    if( m<1.5 ) // snail body
    {
        ...
        f = 1.0-pow( f, 8.0 );
        f = 1.0 - (1.0-f)*(1.0-channel2( 0.3*pos.xy ).x);
	...
	else if( m<2.5 ) // shell
    {
        ...
        vec3 ral = vec3(channel1( vec2(2.0*matInfo.w+matInfo.z*0.5,0.5) ));
        mateD *= 0.25 + 0.75*ral;
    ...
    else if( m<3.5 ) // plant
    {
        ...
        vec3 te = channel2( pos.xy*0.2 ).xyz;
        mateS *= 0.5 + 1.5*te;
    ...
    else //if( m<4.5 ) // leave
    {
        ...
        vec3 te = channel2( p.xz*0.35 ).xyz;
...
vec3 shadeTransparent( in vec3 ro, in vec3 rd, in float t, in float m, in vec4 matInfo, in vec3 col, in float depth ){
    ...
    col += vec3(0.8,1.0,0.8)*0.5*smoothstep(0.3,0.6,channel1(0.8*nor.xy ))*(0.1+0.9*fre*fre);
...
vec3 background( in vec3 d ){
    ...
    for( int i=ZERO; i<200; i++ )
    {
        ...
        vec3 tmp = channel2blur( uv*0.25 + 0.0075*of ).yxz;
...
void mainVR( out vec4 fragColor, in vec2 fragCoord, in vec3 ro, in vec3 rd ){	   
    fragColor = vec4( render( ro, rd, vec2(0.0, 0.0) ), 1.0 );
}