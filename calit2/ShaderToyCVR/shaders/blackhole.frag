...
void mainImage(out vec4 colOut, in vec2 fragCoord) {
    ...
    vec3 ray = normalize(vec3((fragCoordRot-iResolution.xy*.5)/iResolution.x, 1) / iResolution.x);
    ...
    colOut = render(pos, ray);
}

void mainVR(out vec4 fragColor, in vec2 fragCoord, in vec3 fragRayOri, in vec3 fragRayDir ) {
    fragColor = render(fragRayOri + vec3(1.0, 0.0, 1.0), fragRayDir);
}