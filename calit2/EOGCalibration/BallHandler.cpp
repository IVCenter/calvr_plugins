			...
			mBoundingBallGeode(NULL), mCenterBallGeode(NULL)
{
    mBallSwitch = NULL;
    mBallTrans = NULL;
    BOUNDING_BALL_SIZE = cvr::ConfigManager::getFloat("value", "Plugin.EOGCalibration.OuterBall.Size", 0.025);
    CENTER_BALL_SIZE   = cvr::ConfigManager::getFloat("value", "Plugin.EOGCalibration.InnerBall.Size", 0.005);
}
...
CaliBallHandler::CaliBallHandler(MatrixTransform *rootViewerTrans)
{
    ...
    r = cvr::ConfigManager::getFloat("r", "Plugin.EOGCalibration.InnerBall.Color", 1.0f);
    g = cvr::ConfigManager::getFloat("g", "Plugin.EOGCalibration.InnerBall.Color", 1.0f);
    b = cvr::ConfigManager::getFloat("b", "Plugin.EOGCalibration.InnerBall.Color", 1.0f);
    a = cvr::ConfigManager::getFloat("a", "Plugin.EOGCalibration.InnerBall.Color", 1.0f);
    innerBallColor = osg::Vec4(r, g, b, a);

    r = cvr::ConfigManager::getFloat("r", "Plugin.EOGCalibration.OuterBall.Color", 1.0f);
    g = cvr::ConfigManager::getFloat("g", "Plugin.EOGCalibration.OuterBall.Color", 1.0f);
    b = cvr::ConfigManager::getFloat("b", "Plugin.EOGCalibration.OuterBall.Color", 1.0f);
    ...
    const Vec4 &interiorBallColor)
{
   ...
   // bndSphereStateSet->setMode(GL_BLEND, StateAttribute::OVERRIDE | osg::StateAttribute::ON );
    ...
    if (cvr::ConfigManager::getEntry("value", "Plugin.EOGCalibration.OuterBall.Lighting", "off") == "on")
...
/*    Material* ctrSphereMaterial = new Material;
    ...
    if (cvr::ConfigManager::getEntry("value", "Plugin.EOGCalibration.InnerBall.Lighting", "off") == "on")
