							...
							const string &gridUnitLegnthInfo)
{
    ...
    if (gridSVect.x() != 0)
    {
        ...
        idxEnd = gridSVect.x();
        if (gridSVect.x() > 0) 
            gridOffsetVect = Vec3(gridUnitLegnth, 0, 0);
        else 
            gridOffsetVect = Vec3(-gridUnitLegnth, 0, 0);
    }
    ...
    else if (gridSVect.y() != 0)
    {
        ...
        idxEnd = gridSVect.y();
        if (gridSVect.y() > 0) 
            gridOffsetVect = Vec3(0, gridUnitLegnth, 0);
        else 
            gridOffsetVect = Vec3(0, -gridUnitLegnth, 0);
    }
    ...
    else if (gridSVect.z() != 0)
    {
        ...
        idxEnd = gridSVect.z();
        if (gridSVect.z() > 0) 
            gridOffsetVect = Vec3(0, 0, gridUnitLegnth);
        else 
            gridOffsetVect = Vec3(0, 0, -gridUnitLegnth);
    } 
    ...
    if (idxStart == 0) 
        mMoveMatTransVector[0]->setMatrix(mBoundBoxScaleMat * mAccRootMat);
						...
						const std::string &gridUnitScaleInfo)
{
    ...
    /* update the first wireframe with global translation and rotation */
    if (idxStart == 0) 
        mManipulateMatTransVector[0]->setMatrix(mBoundBoxScaleMat * mAccRootMat);
