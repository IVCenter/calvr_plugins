...
void CAVEGeodeSnapWireframeBox::resize(osg::Vec3 &gridVect)
{
    ...
    if (mScaleVect.x() < 0) snapUnitX = -mSnappingUnitDist;
    if (mScaleVect.y() < 0) snapUnitY = -mSnappingUnitDist;
    if (mScaleVect.z() < 0) snapUnitZ = -mSnappingUnitDist;
    ...
    if (mSnapwireGeometry) removeDrawable(mSnapwireGeometry);
    mSnapwireGeometry = new Geometry();
...
void CAVEGeodeSnapWireframeCylinder::resize(osg::Vec3 &gridVect)
{
    ...
    float height = mScaleVect.z(), rad = sqrt(mScaleVect.x() * mScaleVect.x() + mScaleVect.y() * mScaleVect.y());
    int hSeg, radSeg, fanSeg, hDir = 1;
    if (height < 0) { height = -height;  hDir = -1; }
    ...
    if (fanSeg < gMinFanSegments) fanSeg = gMinFanSegments;
	...
	for (int j = 0; j < fanSeg; j++) sideRingStrip->push_back(vertoffset + i * fanSeg + j);
