...
#define VBO_TYPE GL_DYNAMIC_COPY_ARB

#define MAX_VBO_SIZE 150000
...
#ifndef RED_24
    _drawFragFile = _shaderDir + "/draw.frag";
#else
...
void MultiGPUDrawable::drawImplementation(RenderInfo & ri) const
{
    ...
    glFinish();
    getTime(loadend);
    printDiff("Load time: ",loadstart,loadend);
    ...
    getTime(vboDrawEnd);
    std::stringstream ss;
    ss << "VBO Draw ctx:" << context << " ";
    printDiff(ss.str(),vboDrawStart,vboDrawEnd);
	...
	glFinish();
	getTime(estore);
	printDiff("Store Time: ",sstore,estore);
	...
	//getTime(ewait);
	//printDiff("Wait time: ",swait,ewait);
	//
	glFinish();
	...
	for(int i = 0; i < _gpus -1; i++)
	{
	    ...
	    memcpy(buffer,_colorDataMap[i+1],_width*_height*4);
	    glUnmapBuffer(GL_PIXEL_UNPACK_BUFFER_ARB);*/
	    //glTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, _width, _height, GL_RGBA, GL_UNSIGNED_BYTE, BUFFER_OFFSET(0));
	...
	glFinish();
	getTime(cbend);
	printDiff("CopyBackTime: ", cbstart,cbend);
	...
	glFinish();
	getTime(shaderend);
	printDiff("shader time: ",shaderstart,shaderend);
...
void MultiGPUDrawable::initBuffers(int context) const
{
    ...
    glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_DEPTH_COMPONENT24, _width, _height, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
