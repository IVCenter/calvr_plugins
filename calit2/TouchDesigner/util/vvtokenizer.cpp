...
/** Constructor.
  ...
  <LI>EOL is not significant</LI>
  <LI>Numbers are not parsed</LI></UL>
  @param char pointer, equivalent for line in 
*/
vvTokenizer::vvTokenizer(char * cData)
{
  // Memory allocation:
  sval = new char[MAX_TOKEN_LEN + 1];
  //data = new char[BLOCK_SIZE];
...
void vvTokenizer::reset()
{
	cur = 0;

}

void vvTokenizer::newData(char* ndata)
{
	data = ndata; 
	cur = 0;
}

int vvTokenizer::currentPos()
{
	return cur;
}

char * vvTokenizer::peekData()
{
	return data;
}
//============================================================================
