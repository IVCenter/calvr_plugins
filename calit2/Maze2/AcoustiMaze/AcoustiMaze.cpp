...
AcoustiMaze::AcoustiMaze(/*MaxObject mo*/)
{
    ...
    std::vector<int> row0, row1, row2, row3;
    row0.push_back(0);
    row0.push_back(1);
    chk.push_back(row0);
    row1.push_back(1);
    ...
    std::vector<MazeSquare*> tmpVec;
    for(int i = 0; i < 20; i++)
    {
        for(int j = 0; j < 20; j++)
        {
            tmpVec.push_back(new MazeSquare(this, i, j));
        }
        std::vector<MazeSquare*> vec(tmpVec);
        maze.push_back(vec);
        tmpVec.clear();
    }
...
void AcoustiMaze::loadFile(std::string filename)
{
    std::ifstream inFile;
    inFile.open(filename.c_str());
    if (!inFile) 
    {
        std::cout << "Unable to open maze model file " << filename << std::endl;
        return;
    }
    ...
    while (inFile >> entryNameStr)
    {
        ...
        else if (!strcmp(entryNameStr.c_str(), "Close_Wall"))
        {
            ...
            maze[sqx - xo][sqy - yo]->walls[wallNum] = "Close_Wall";
        }
        else if (!strcmp(entryNameStr.c_str(), "Open_Wall"))
        {
            ...
            maze[sqx - xo][sqy - yo]->walls[wallNum] = "Open_Wall";
        }
...
void AcoustiMaze::updatePoses(int type, float x, float y, float z, float yaw, float pitch, float roll)
{
    if (type == 0) 
    {
        obs.set(x, y , z, yaw, pitch, roll); 
        eval(obs, 0);
    }
    if (type == 1) 
    {
        src.set(x, y , z, yaw, pitch, roll); 
        eval(src, 1);
    }
}
...
{
    _mainMenu = new cvr::SubMenu("AcoustiMaze");
    cvr::PluginHelper::addRootMenuItem(_mainMenu);

    _loadBtn = new cvr::MenuButton("Load");
    _loadBtn->setCallback(this);
    _mainMenu->addItem(_loadBtn);

    _clearBtn = new cvr::MenuButton("Clear");
    _clearBtn->setCallback(this);
    _mainMenu->addItem(_clearBtn);
...
void AcoustiMaze::check_test(int squareX,int squareY,int enterW,int gen)
{
...
void AcoustiMaze::check(int squareX,int squareY,int enterW,int gen) //check the square, entering from wall enterW, step gen.
{
    int wall=0;
    for (int w=0;w<4;w++) //check wall w (N,E,S,W)
    {
        ...
        if ((!(enterW==w)) || (gen==0)) //if w is not where we came in, and this is not the square with the src
        {
            ...
            {
                isInPathAlready=isInPathAlready||((maze[squareX][squareY]->inPath[enterW]->pts[op][0] == inX)
                  && (maze[squareX][squareY]->inPath[enterW]->pts[op][1] == inY)); 
            }    
             ...
             if (inX>=0 && inX<xs && inY>=0 && inY<ys && !isInPathAlready)
             {
                ...
                if (wall==0) 
                {
                    ...
                    if (gen==0)
                    {
                        ...
                        maze[inX][inY]->seesDirect = maze[inX][inY]->seesDirect || 
                            isStraightLine(maze[inX][inY]->inPath[iw]->pts, maze[inX][inY]->inPath[iw]->pl);
                    ...
                    else if (maze[inX][inY]->inPath[iw]->pl > maze[squareX][squareY]->inPath[enterW]->pl || 
                             maze[inX][inY]->reached[iw] == false) 
                    {
...
bool AcoustiMaze::isStraightLine(std::vector< std::vector<int> >& line, int l)
{
    xsum=0;
    ysum=0;
    bool result=false;
    ...
    result=(abs(xsum)==l-1)||(abs(ysum)==l-1);
