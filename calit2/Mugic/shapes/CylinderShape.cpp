...
/* Constructor */
CylinderShape::CylinderShape(std::string command, std::string name)
{

	_type = SimpleShape::CYLINDER;
	BasicShape::setName(name);

	//create cylinder and add to geode as shape drawable
...
/* set position of cylinder - radius and height */
void CylinderShape::setPosition(osg::Vec3 position, float radius, float height)
{

	_cylinder->setRadius(radius);
	_cylinder->setHeight(height);
	_cylinder->setCenter(position);
...
/* set color of cylinder */
void CylinderShape::setShapeColor(osg::Vec4 color)
{
...
/* set shaders of cylinder */
void CylinderShape::setShaders(std::string vert_file, std::string frag_file)
{
...
/* update with command */
void CylinderShape::update(std::string command)
{
