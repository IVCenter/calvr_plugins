...
void VortexCoresVisMode::initContext(int context)
{
...
void VortexCoresVisMode::uinitContext(int context)
{
    pthread_mutex_lock(&_shaderInitLock);
	...
	glDeleteProgram(_vortexAlphaProgram[context]);
...
void VortexCoresVisMode::draw(int context)
{
    ...
    std::vector<FlowPagedRenderer::AttribBinding> vcoreAttribBinding;
    std::vector<FlowPagedRenderer::TextureBinding> vcoreTexBinding;
    std::vector<FlowPagedRenderer::UniformBinding> vcoreUniBinding;
    ...
    if(set->frameList[currentFrame]->vcoreVerts.first > 0 && set->frameList[currentFrame]->vcoreSegments.size())
    {
	drawCores = true;

	corePointVBO = cache->getOrRequestBuffer(context,fileID,set->frameList[currentFrame]->vcoreVerts.second,set->frameList[currentFrame]->vcoreVerts.first*3*sizeof(float),GL_ARRAY_BUFFER);
	coreStrVBO = cache->getOrRequestBuffer(context,fileID,set->frameList[currentFrame]->vcoreStr.second,set->frameList[currentFrame]->vcoreStr.first*sizeof(float),GL_ARRAY_BUFFER);

	FlowPagedRenderer::UniformBinding ub;
	ub.location = _vortexAlphaMinUni[context];
	ub.type = uniDataMap["vmin"].type;
	ub.data = uniDataMap["vmin"].data;
	vcoreUniBinding.push_back(ub);
	ub.location = _vortexAlphaMaxUni[context];
	ub.type = uniDataMap["vmax"].type;
	ub.data = uniDataMap["vmax"].data;
	vcoreUniBinding.push_back(ub);
	...
	glEnable(GL_BLEND);
	for(int i = 0; i < set->frameList[currentFrame]->vcoreSegments.size(); ++i)
	{
	    _renderer->drawArrays(GL_LINE_STRIP,set->frameList[currentFrame]->vcoreSegments[i].first,set->frameList[currentFrame]->vcoreSegments[i].second,corePointVBO,color,vcoreAttribBinding,vcoreProg,vcoreTexBinding,vcoreUniBinding);
	}
	glDisable(GL_BLEND);
    }
	...
	if(set->frameList[nextFrame]->vcoreVerts.first > 0 && set->frameList[nextFrame]->vcoreSegments.size())
	{
	    withCores = true;
	    vcpbuf = cache->getOrRequestBuffer(context,nextfileID,set->frameList[nextFrame]->vcoreVerts.second,set->frameList[nextFrame]->vcoreVerts.first*3*sizeof(float),GL_ARRAY_BUFFER);
	    vcsbuf = cache->getOrRequestBuffer(context,nextfileID,set->frameList[nextFrame]->vcoreStr.second,set->frameList[nextFrame]->vcoreStr.first*3*sizeof(float),GL_ARRAY_BUFFER);
	}

	bool nextReady;
	if(ibuf && vbuf && (!nextattrib || abuf) && (!withCores || (vcpbuf && vcsbuf)))
	{
